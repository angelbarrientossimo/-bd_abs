...
```python
df_renamed = df.select(col("Crop"),col("Region"),col("Temperature_C").alias("Temperatura"),col("Rainfall_mm").alias("Lluvia"),col("Irrigation"),col("Yield_ton_per_ha").alias("Rendimiento"))
```
...
```python
df_renamed.where(col("Crop") == "Maize").show(3)
```
...
```python
df_renamed.where(col("Temperatura") > "25").show(3)
```
...
```python
df_base = df.select(col("Place_Name"),col("Country"),col("UNESCO_World_Heritage"),col("Entry_Fee_USD"),col("Annual_Visitors_Millions"))
df_base.show(3)
```
...
```python
df_es = df.select(col("Place_Name").alias("Lugar"),col("Country"),col("UNESCO_World_Heritage").alias("Es_UNESCO"),col("Entry_Fee_USD").alias("Precio_Entrada"),col("Annual_Visitors_Millions").alias("Visitantes_Millones"))
df_es.show(3)
```
...
```python
df_contactos = df.select(col("Nombre"),col("Tipo"),col("Provincia"),col("web"),col("Email"))
...
```python
df_limpio = df_contactos.select(col("Nombre").alias("nombre_establecimiento"),col("Tipo").alias("categoria_actividad"),col("Provincia"),col("web").alias("sitio_web"),col("Email").alias("correo_electronico"))
df_limpio.show(3)
```
