...
```
    ...
    26/01/21 12:35:08 WARN NativeCodeLoader: Unable to load native-hadoop library for your platform... using builtin-java classes where applicable
...
```
    ...
    26/01/21 12:35:27 WARN GarbageCollectionMetrics: To enable non-built-in garbage collector(s) List(G1 Concurrent GC), users should configure it(them) to spark.eventLog.gcMetrics.youngGenerationGarbageCollectors or spark.eventLog.gcMetrics.oldGenerationGarbageCollectors
...
```
    ...
    |          Colosseum|        Italy|            Rome|                    NULL|  Historic Monument|                  Yes|        80|           18| Apr-May/Sept-Oct|Southern Europe|                         85|                         2.5|Ancient Roman amp...|
    |  Statue of Liberty|United States|   New York City|                    NULL|    Monument/Statue|                  Yes|      1886|           25|         May-Sept|  North America|                         45|                         2.0|Symbol of freedom...|
    | Sydney Opera House|    Australia|          Sydney|                    NULL|  Cultural Building|                  Yes|      1973|           49|         Sept-Nov|        Oceania|                        110|                         1.5|Unique sail-like ...|
    |       Machu Picchu|         Peru|    Cusco Region|                    NULL|Archaeological Site|                  Yes|      1450|           70|          Apr-Oct|  South America|                        180|                         8.0|Ancient Incan cit...|
    +-------------------+-------------+----------------+------------------------+-------------------+---------------------+----------+-------------+-----------------+---------------+---------------------------+----------------------------+--------------------+
...
```
    ...
    26/01/21 12:36:26 WARN SparkStringUtils: Truncated the string representation of a plan since it was too large. This behavior can be adjusted by setting 'spark.sql.debug.maxToStringFields'.
